<?php
    ...
    // File upload
    $upload_dir = 'uploads/visiting_cards/';
    ...
    $sql =  "INSERT INTO cpr_reqiuis_master (Req_for , staffno , dept_code , category_id , cateory_desc , sub_category , contact_no , purpose_justif , file_path , add_by , add_dt , leg_id ) 
        VALUES ('$staff_info' , '$staffno' , '$dept_code' , 8 , 'Visiting_Card' , '$category' , '$contact' , '$justification' , '$destination' , '$staffno' , CURDATE() , 1)";

    mysqli_query($conn , $sql);

    mysqli_close($conn);

    header('Location: visitingcard.php?submitted=true');
    exit();
