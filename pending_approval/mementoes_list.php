<?php
require_once('../auth.php');
authorize([3,4]); // Only CPR
include('../database.php');


if (!isset($_SESSION['staffno'])) {
    header("Location: ../login.php");
    exit;
}

$query = "SELECT Req_id, add_dt, sub_category, item_desc, purpose_justif 
          FROM CPR_REQIUIS_MASTER 
          WHERE category_id = 2 AND leg_id = 2 
          ORDER BY add_dt DESC";

$result = mysqli_query($conn, $query);
?>

<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    <title>Mementoes & Gifts Requests - Pending Approval</title>
    <link href="../output.css" rel="stylesheet">
</head>
<body class="bg-gradient-to-br from-green-200 via-white to-blue-200 min-h-screen font-sans">
<?php include('../components/navbar.php'); ?>

<main class="max-w-6xl mx-auto px-6 py-12">
    <div class="bg-white shadow-2xl rounded-2xl border border-blue-300 p-8">
        <h1 class="text-3xl font-bold text-blue-900 mb-8 text-center">🎁 Mementoes & Gifts - Pending Requests</h1>

        <div class="overflow-x-auto rounded-lg shadow-inner">
            <table class="min-w-full divide-y divide-gray-300 bg-white text-sm text-gray-800">
                <thead class="bg-blue-500 text-white text-left text-sm uppercase tracking-wider">
                    <tr>
                        <th class="px-4 py-3">Req. No.</th>
                        <th class="px-4 py-3">Request Date</th>
                        <th class="px-4 py-3">Sub Category</th>
                        <th class="px-4 py-3">Item Description</th>
                        <th class="px-4 py-3">Purpose / Justification</th>
                        <th class="px-4 py-3 text-center">Action</th>
                    </tr>
                </thead>
                <tbody class="divide-y divide-gray-100">
                    <?php if (mysqli_num_rows($result) > 0): ?>
                        <?php while ($row = mysqli_fetch_assoc($result)): ?>
                            <tr class="hover:bg-gray-50">
                                <td class="px-4 py-3"><?= htmlspecialchars($row['Req_id']) ?></td>
                                <td class="px-4 py-3"><?= date("d M Y", strtotime($row['add_dt'])) ?></td>
                                <td class="px-4 py-3"><?= htmlspecialchars($row['sub_category']) ?></td>
                                <td class="px-4 py-3"><?= htmlspecialchars($row['item_desc']) ?></td>
                                <td class="px-4 py-3"><?= htmlspecialchars($row['purpose_justif']) ?></td>
                                <td class="px-4 py-3 text-center">
                                    <a href="mementoes_view.php?req_id=<?= $row['Req_id'] ?>"
                                       class="inline-block bg-blue-600 hover:bg-blue-700 text-white text-xs font-semibold px-4 py-2 rounded-full shadow transition">
                                        View
                                    </a>
                                </td>
                            </tr>
                        <?php endwhile; ?>
                    <?php else: ?>
                        <tr>
                            <td colspan="6" class="text-center py-6 text-gray-500">No pending Mementoes requests.</td>
                        </tr>
                    <?php endif; ?>
                </tbody>
            </table>
        </div>
    </div>
</main>
</body>
</html>
